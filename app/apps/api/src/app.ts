// Fastify assembly — The One API. All routes under /api.
//
// No CORS: in dev the browser reaches this through the Vite proxy, and on
// Vercel the SPA and the function share one origin — every request is
// same-origin either way. That is also what makes a SameSite=Lax session
// cookie work without any cross-site relaxation.
//
...
//
// Split out of index.ts so two entrypoints can share it verbatim:
//   index.ts        local dev — listens on a port, starts the scheduler
//   app/api/*       Vercel    — hands requests to the instance, never listens
...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  // Refuses to run in DEMO_MODE (see routes/integrations.ts).
