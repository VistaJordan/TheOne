...
/** Non-canonical sample status strings → canonical status name. The legacy
    ClickUp/Ecotrak vocabulary (lowercase, '!!'-prefixed) maps onto the
    operational vocabulary here, so older seed sections and sample data keep
    resolving after the S8 rename. */
const STATUS_ALIAS: Record<string, string> = {
  'emergency': 'Emergency',
  'assessment scheduled': 'Assessment Sched',
  'job scheduled': 'Job Sched',
  'pm scheduled': 'PM Sched',
  'assessment ongoing': 'On Site (Assessment)',
  'job ongoing': 'On Site (Job)',
  'return trip needed': 'Return Trip Needed',
  'waiting for quote': 'Waiting for Quote',
  'quote ready': 'Quote Ready',
  'waiting for parts': 'Waiting for Parts',
  'please order parts': 'Please Order Parts',
  '!! waiting for advice': 'Waiting for Advice',
  '!! waiting for approval': 'Waiting for Approval',
  'approved': 'Approved',
  '!! approved': 'Approved',
  '!! ready to invoice': 'Ready to Invoice',
  'done/incurred': 'Done / Incurred',
  '<< invoiced not paid >>': 'Invoiced Not Paid',
  'invoiced': 'Invoiced',
  '!! canceled/postponed': 'Cancelled / Postponed',
};
...
const CURATED_FIELDS: CuratedField[] = [
  ...
  // migration 0024 carries the SAME two rows for an already-seeded pgdata.
...
async function main() {
  ...
  // ── 3. Statuses (§4.2) ─────────────────────────────────────────────────────
  // The five built-in phase groups, kept in step with migration 0013 (the seed
  ...
  // The "blocked on someone outside dispatch" statuses live in the 'pending'
  // group (migration 0006/0007's split): the client owes a decision, a
  // supplier owes delivery — no dispatcher effort advances them.
  const PENDING_STATUSES = new Set(['Waiting for Approval', 'Waiting for Advice', 'Waiting for Parts']);

  const statusIdByName = new Map<string, string>();
  const statusGroupById = new Map<string, StatusGroup>();
  for (const s of data.statuses) {
    const group = PENDING_STATUSES.has(s.name) ? 'pending' : STATUS_GROUP_BY_TYPE[s.type];
  ...
  // Archive terminal status "Invoiced" (20th, slotted between Done / Incurred
  // and Invoiced Not Paid — the JSON skips order 17 for it) — §4.2 / §10
  // decision. Purple (the palette's #b660e0) rather than the spec's grey, per
  // founder request.
  {
    const id = await insertId(
      `INSERT INTO status (status_set_id, name, status_group, color, position, is_archive)
       VALUES ($1, 'Invoiced', 'done', '#b660e0', 17, true) RETURNING id`,
      [statusSetId],
    );
